...
class Strategy(metaclass=ABCMeta):
    ...
    def buy(self, *,
        ...
        """
        #assert 0 < size < 1 or round(size) == size, \ #modified
...
class Position:
    ...
    def close(self, portion: float = 1., tradeid=0):
        """
        Close portion of position by closing `portion` of each active trade. See `Trade.close`.
        """
        # if tradeid != 0:
        #     self.__broker.trades[tradeid - 1].close(portion) #modified para HEDGING cerrar solo un trade y no todas las posiciones
        # else:
...
class Trade:
    ...
    def __repr__(self):
        return f'<Trade size={self.__size} time={self.__entry_bar}-{self.__exit_bar or ""} ' \
               f'price={self.__entry_price}-{self.__exit_price or ""} pl={self.pl:.0f} tp={self.__tp_order} sl={self.__sl_order}>' #modified
    ...
    def pl(self):
        ...
        #original: return self.__size * (((price - self.__entry_price) * self.__broker._cash) / self.entry_price)
        return (self.__broker._leverage * (self.__broker._cash / self.__broker._maxtrades)) * (self.pl_pct - self.__broker._commission) #modified
...
class _Broker:
    def __init__(self, *, data, cash, commission, margin,
                 trade_on_close, hedging, exclusive_orders, index, maxtrades): #modified maxtrades added
        ...
        self._maxtrades = maxtrades #modified
    ...
    @property #modified all new
    def maxtrades(self) -> int:
        return self._maxtrades
    ...
    def _close_trade(self, trade: Trade, price: float, time_index: int):
        ...
        #self._cash += self._cash * trade.pl_pct FUNCIONA MEDIAS modified
        self._cash += trade.pl
...
class Backtest:
    ...
    def __init__(self,
        ...
        self._broker = partial(
            ...
            exclusive_orders=exclusive_orders, index=data.index, maxtrades=maxtrades #modified
        )
