...
return this.labels[index] || "";
    ...
    def _plot_ohlc_trades():
        ...
        fig_ohlc.multi_line(xs='position_lines_xs', ys='position_lines_ys',
                            ...
                            #legend_label=f'Trades ({len(trades)})  WR Total (%{(((trades.ReturnPct.gt(0).sum())*100)/len(trades)).round(2)}) / WR Long (%{round((((sum((trades.Size == 1) & (trades.ReturnPct > 0)))*100)/sum(trades.Size == 1)),2)}) ({sum((trades.Size == 1) & (trades.ReturnPct > 0))} de {sum(trades.Size == 1)}) / WR Short (%{round((((sum((trades.Size == -1) & (trades.ReturnPct > 0)))*100)/sum(trades.Size == -1)),2)}) ({sum((trades.Size == -1) & (trades.ReturnPct > 0))} de {sum(trades.Size == -1)})',
